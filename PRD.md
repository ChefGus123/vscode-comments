...
**Creating a comment from the Markdown preview (`agentComments.addCommentFromPreview`):**

Right-clicking a block in VS Code's *built-in* Markdown preview offers **Add Comment**, which anchors to the corresponding line of the source `.md`. We extend the built-in preview only; shipping our own preview webview is a non-goal (§2). Comments *are* now rendered inside the preview too — see "Comment presence + actions in the Markdown preview" below; that was a deliberate later addition, not a reversal of a design principle here.

The mechanism, because none of it is obvious:
- `contributes["markdown.previewScripts"]` injects `media/preview.js` into the preview. It is **not** possible to message the extension host from there — the built-in preview already called `acquireVsCodeApi()`, which is once-per-webview (vscode#122961, closed as-designed). The design therefore never needs a webview→host channel.
- Instead, `preview.js` writes a `data-vscode-context` JSON blob onto the right-clicked element, and a `webview/context` menu item gated on `webviewSection == 'agentCommentsPreviewTarget'` fires a normal command **in the extension host** with that blob forwarded as its argument. `webview/context` is a global menu id, not scoped to the webview's owning extension — the built-in `mermaid-markdown-features` contributes into `markdown-language-features`' preview the same way.
- Core reads `data-vscode-context` **at contextmenu time** (bubble-phase listener on the webview iframe's window, merging `closest()` ancestors inner-most-first), so our capture-phase listener on `document` is guaranteed to run first and can set the attribute live — that's what makes the current text selection available in the payload. Corollary: `preview.js` must never call `preventDefault()`, since core's handler bails on `defaultPrevented`.
- **Which file the preview is showing is not a heuristic.** The preview embeds `<meta id="vscode-markdown-preview-data" data-settings="...">` whose JSON carries `source` — the document URI. `preview.js` reads it straight out and puts it in the payload, so the host never has to guess from the active editor or prompt between open `.md` files.
- **Line numbers come from the preview's own scroll-sync source map**: markdown-it's `source_map_data_attribute` rule sets `data-line="<zero-based line>"` + `class="code-line"` on *every* non-inline token with a map. (An older limitation, vscode#133376, restricted this to a short list of block types that excluded tables; current VS Code no longer does.) `data-line` is zero-based, `anchor.lineHint` is one-based — the handler adds 1. The preview also appends a sentinel `data-line="${lineCount}"` div past the end of the body, so the host clamps into the document. If no `.code-line` ancestor exists, `preview.js` falls back to the nearest *preceding* mapped element and never guesses forwards; if nothing resolves, it sets no context at all and the menu item simply doesn't appear, rather than anchoring somewhere wrong.
- Unlike the editor paths, this uses an input box rather than a draft `CommentThread` — the preview isn't a `TextEditor`, so there's no gutter to draw one in. Everything downstream (`createAnchor` → `store.addComment`, `author.type: "user"`) is the existing pipeline unchanged; a preview-created comment is indistinguishable in storage from a gutter-created one.

**Multi-line selections from the preview — "snap and show".** Selecting across blocks and right-clicking comments on the whole span. Storage needed no change for this: `Anchor` has carried `lineHint` + `endLineHint` since the beginning, and the editor's own selection path already used them.

- **The unit is the block, not the character.** markdown-it's source map is block-level — only top-level, non-`inline` tokens get `data-line`, so inline content carries no position at all. Selecting three sentences out of a five-line paragraph therefore *cannot* be honored; the anchor widens to the whole paragraph. Rather than hide that, the input box states the resulting range (`Lines 20–27: …`) before the user commits to a comment about it. Widening (never narrowing) is the deliberate direction: an under-covering range reads as an off-by-a-few bug, an over-covering one reads as "it took the blocks I touched."
- **A bare right-click stays single-line**, and only a real selection widens. Not an inconsistency for its own sake: a `contentHash` over one line survives edits elsewhere in the same block, so the narrower anchor is materially more durable, and pointing at a spot is a weaker signal of intent than dragging across one.
- **Deriving the end line.** `data-line` says where a block *starts*; nothing says where it ends. `endLineOf` takes the smallest `data-line` in the document greater than the end block's own, minus one. Two details make this safe: the preview appends a sentinel `data-line="${lineCount}"` div after the body, so even the final block has a "next"; and the comparison is *strictly* greater, because a `<ul>` and its first `<li>` share a line and would otherwise yield an end before the start. Scanning the whole document for the minimum (rather than walking forward) also stays correct if a plugin renders a block out of source order. The blank separator line this leaves on the end is trimmed host-side, where the real document text is available.
- **`getRangeAt(0)` is always normalized start-before-end**, so a bottom-to-top drag needs no special handling.
- **Whether the click landed inside the selection** can't be read off the selection itself: Chromium collapses a selection when you right-click outside it, but not before our capture-phase handler runs. The test used instead is that the block under the cursor falls within the selection's own block span — reusing the element we already had to resolve for the menu host.

**Comment presence + actions in the Markdown preview.** Builds on the add-from-preview mechanism above with a second, independent mechanism for the reverse direction — showing that a comment already exists, and acting on it, from inside the preview itself.

- **Rendering mechanism:** `contributes.markdown.markdownItPlugins: true` + `activate()` returning `{ extendMarkdownIt(md) { ... } }` — a markdown-it plugin registered via `md.core.ruler.push`, run in the **extension host** on every render, with direct synchronous access to `CommentStore`. This is a genuinely different mechanism from `preview.js`'s `previewScripts` injection (which runs in the webview and only annotates the DOM at click time): the plugin marks up the rendered HTML itself, before it ever reaches the webview.
- **Which file:** VS Code's own markdown engine passes `env.currentDocument` (a `vscode.Uri`) to every render — confirmed by reading the engine's actual source (`extensions/markdown-language-features/src/markdownEngine.ts`). Converted to a storage key with the same `toWorkspaceRelativePath` every other entry point uses. **This turned out not to be reliable in practice**, discovered against a real VS Code build (source-reading alone missed it — see §13's entry on this): `env.currentDocument` is only set when the render call is given an actual document object (`typeof input === 'string' ? undefined : input.uri`); a preview panel revived after a window/extension reload was observed re-rendering from a plain string every time, leaving `currentDocument` present as an `env` key but `undefined` as a value on *every* render pass, indefinitely — not just the first cold one. Fixed with a fallback: `extension.ts` tracks the last-focused Markdown editor (`vscode.window.onDidChangeActiveTextEditor`, seeded from `vscode.window.activeTextEditor` at activation) and the plugin falls back to it whenever `env.currentDocument` is undefined. Imprecise for a *locked* preview showing a file other than the active editor (same tradeoff the original, pre-DOM-heuristic `addCommentFromPreview` design accepted, before the better `<meta>`-embedded-URI heuristic was found for that feature) — accepted, since it's far better than never rendering markers after a reload.
- **Synchronous cache, async warm-up.** `CommentStore.loadFile`/`getArchivedComments` are async (real disk I/O on a cold cache); a markdown-it core rule is not. `CommentStore.peekCachedComments(filePath)` reads straight from the store's existing in-memory `cache`/`archiveCache` maps with no I/O, returning `undefined` per side when cold. A cold render shows no markers for that pass and fires the real async load in the background; once it resolves, `markdown.preview.refresh` is called so a follow-up render picks the markers up. The same refresh path is reused for live comment-store changes generally — a comment being added/resolved/deleted anywhere (gutter, sidebar, MCP) never touches the `.md` file's own text, so VS Code's preview has no native trigger to re-render on its own; without an explicit `store.onDidChangeFile` → debounced `markdown.preview.refresh` subscription (in `extension.ts`), markers would silently go stale the moment a comment is acted on from anywhere else.
- **Matching a comment to a block.** `anchor.lineHint`/`endLineHint` are **1-based** in this codebase; markdown-it's `token.map` is genuinely 0-based — converted before comparing, not compared raw. For each comment, the *innermost* block whose `.map` range fully contains it wins (markdown-it block ranges are strictly hierarchical — a child's range is always fully inside its parent's, never overlapping a sibling's — so "smallest containing range" is a deterministic choice, not a heuristic; a same-width tie, e.g. a tight single-item list where the list/item/content all share one range, breaks toward the more deeply nested token, since a parent's opening token is always emitted before its child's). If no single block contains a comment's full span (a multi-block gutter selection with no common container — e.g. two adjacent top-level paragraphs with nothing wrapping them), it falls back to every block the comment overlaps, excluding any block that's itself an ancestor of another overlapping block for that same comment (so a wrapping list isn't marked redundantly alongside the items already representing it). Pure and markdown-it-free (`matchCommentsToBlocks` in `src/preview/commentsMarkdownItPlugin.ts`), unit tested directly.
  - **`inline` tokens are excluded from the candidate block list before matching, not an edge case of the algorithm above.** Found against a real preview (comments matched and attributes were genuinely being set — confirmed via diagnostic logging — yet nothing ever rendered): an `inline` token always shares its *exact* `.map` range with its immediate parent block token (`paragraph_open`+its `inline` child, `heading_open`+its `inline` child, etc.) — a same-width tie the tie-break rule above resolves toward the later, "more nested" token, which is the `inline` one. But markdown-it's renderer special-cases `type === 'inline'`: it renders the token's `.children` directly and never reads the inline token's own `.attrs` at all, so `attrJoin`/`attrSet` on one is a silent no-op — real match, real attribute call, zero HTML output. Fixed by never treating an `inline`-typed token as a candidate block in the first place (mapped to `null` before matching runs), so the tie-break rule never has to arbitrate between a block and its own inline child.
- **Visual language reused, not reinvented — and non-intrusive by construction.** The marker is one small circular badge in the exact colors the gutter's author-avatar SVGs already use (`#3794ff` user / `#f14c4c` agent), a two-tone split when a block has both a user and an agent comment, a plain dot for one comment or the count digit(s) for more. Resolved-comment visibility respects the existing `agenticComments.editor.hideResolvedComments` setting, the same as the gutter. The native `title` attribute carries a plain-text summary for hover-to-read, no custom tooltip component.
  - **The badge is a genuinely separate DOM element, not a decoration on the block itself — this took two attempts.** First try: a CSS `::before` pseudo-element on the marked block, `display: inline-block`. Participates in normal flow — visibly pushed table columns and paragraph text around ("tables get destroyed entirely," reported directly). Second try: same pseudo-element, switched to `position: absolute`, reasoning that removing it from layout should make it harmless regardless of host element. Still distorted tables in practice — table row/cell positioning has cross-engine quirks in how `position` on or inside them is handled that don't show up from reasoning about the CSS spec alone, and this was reported back as still broken. Fixed by abandoning the pseudo-element approach entirely: the badge is now a real `<span>`, built and positioned by `preview.js` (`getBoundingClientRect()` on the block, offset applied, appended to `<body>`) — the same technique the click-to-expand panel already used, for the same reason (a block can be a `<p>`/`<li>`/`<td>`/heading/`<tr>`, and a marker living entirely outside its DOM subtree cannot affect its layout, categorically, regardless of how any given engine handles positioning *inside* one). `commentsMarkdownItPlugin.ts` and its matching logic are unaffected by any of this — `class="agent-comment-line"` plus the `data-agent-comment-*` attributes it sets are now a pure query-selector hook for `preview.js` to find commented blocks and read their data from, carrying no CSS styling of their own.
  - **Kept in sync across re-renders via `MutationObserver`, not polling.** Every badge is torn down and rebuilt (`syncMarkers()`) whenever the rendered document's content actually changes — detected by observing `document.body` for `childList`/`subtree` mutations, debounced to one pass per animation frame so a single re-render's flurry of DOM changes doesn't trigger redundant rebuilds. Since `syncMarkers()` itself mutates `<body>` (appending badges), the observer is disconnected for the duration of a rebuild and reconnected after, to avoid it retriggering on its own output.
- **One marker per comment, never one per line it touches.** The multi-block fallback (§ above) originally marked *every* block a comment overlapped — for a comment spanning many short blocks (a table's rows, a list's items), that painted a badge on every single one, reported as "dots every single line" and "a complete shambles." Changed to mark only the earliest-starting of those blocks — one badge for the whole span, matching how the gutter itself only ever shows one indicator per comment regardless of how many lines it covers.
- **Click-to-expand, in place, no navigation.** Clicking a marked block toggles a small panel showing every comment on it (author dot, author, status, text) directly in the preview — explicitly *not* a re-implementation or a reveal of VS Code's real `CommentThread` widget, which is an editor-only construct with no API to embed in a webview. Its expand/collapse state is local to that preview instance only; nothing about it is persisted, since this extension has never persisted thread-collapse state anywhere (a real constraint discovered while designing this, not a shortcut). The panel is a single floating element positioned over the clicked block via `getBoundingClientRect()`, appended to `<body>` rather than inserted as a child/sibling of the marked element itself — a marked block can be a `<p>`, `<li>`, `<td>`, or heading, none of which can validly contain (or sit beside, inside their own parent) an arbitrary block-level child without the browser silently renormalizing the DOM.
  - **Survives the refresh acting on a comment triggers.** Every store change (including one made from inside this very panel) fires `markdown.preview.refresh`, which re-renders the whole document — every element in it, including whichever one the panel was anchored to, gets discarded and replaced with a new one. Reported directly: resolving one comment out of several shown in an open panel was silently closing the panel entirely, hiding the *other*, untouched comments in it too. Fixed by tracking `openCommentIds` (the actual comment ids on display) rather than only the host element — the same `MutationObserver`-driven resync described above also notices when `openHost` is no longer connected to the document (the tell that a refresh just replaced it) and looks for a freshly-rendered element carrying any of the same ids, reopening the panel there with whatever's left; if none of them exist anymore (all resolved-and-hidden or deleted), it closes instead of lingering on stale content.
- **Right-click actions: Edit / Resolve / Reopen / Delete**, wired to the exact same store primitives the gutter and sidebar already call (`store.updateCommentText`, `store.resolveComment`, `store.reopenComment`, `store.deleteComment`) — no new comment-mutation logic. **Reopen** wasn't in the original request for this feature, which named only edit/resolve/delete; added anyway, since resolve/reopen are a symmetric pair at every other entry point in this codebase (gutter, sidebar), and without it a resolved comment shown in the preview (`hideResolvedComments: false`) would have no way back to unresolved except leaving the preview. **Edit** can't literally reuse the gutter's `editComment`/`saveComment` (those flip a live `CommentThread`'s comment into `CommentMode.Editing` — a construct that doesn't exist in a webview); it reuses the primitive underneath that instead (`store.updateCommentText`, which itself only ever looks at live/unresolved comments — so edit is unresolved-only here too, same as the gutter), via an input box prefilled with the current text, mirroring `addCommentFromPreview`'s own input-box pattern. All resolve which file the same way `addCommentFromPreview` already does (the preview's own embedded `source` URI, no guessing) — one shared helper, not several copies.
  - **Two right-click entry points, same commands, same controller code — and, since the badge moved to its own DOM element, no overlap with "Add Comment" to gate around anymore.** (1) Right-click the floating badge itself (`webviewSection: 'agentCommentsMarkerTarget'`) — carries every comment id on that block; if more than one is applicable, `showQuickPick` disambiguates (label = truncated text, description = author + status), a lone candidate acts without asking. (2) Right-click a *specific row* inside the already-open expand panel (`'agentCommentsPanelRowTarget'`) — carries just that one id, so the quickpick never appears at all. Reported directly as much better UX than picking a target out of a list after right-clicking an ambiguous line. Both wire to the identical `editCommentFromPreview`/etc. commands and `resolvePreviewCommentTarget` controller logic; a row's context payload is just a single-id case of the same shape the marker's already produces, so no controller changes were needed for either. Right-clicking real document content (a paragraph, a table cell) now offers only **Add Comment** (`'agentCommentsPreviewTarget'`, unchanged) — there's no longer a shared element where both that and the four actions could plausibly appear together, since the badge lives outside the content entirely. `package.json`'s `webview/context` `when` clauses still pre-filter which actions even appear (Resolve only when there's an unresolved candidate in scope, Reopen only a resolved one) using two boolean-ish flags the plugin/preview.js compute per scope — the controller still does its own precise per-id filtering once an action actually runs, since a stale marker could theoretically lag one render behind a change made elsewhere.
  - **True left-click action buttons are not achievable here**, and this was checked rather than assumed: `data-vscode-context` is wired exclusively to VS Code's native context-menu (right-click) system, with no left-click equivalent contribution point, and synthesizing a `contextmenu` event from a `click` handler is unreliable (browsers generally refuse to honor an untrusted, script-dispatched one for security reasons). Row-scoped right-click (above) is the achievable version of "act on the comment you mean, not an ambiguous line."
...
| Add Comment (Markdown preview) | Right-click a block in the built-in Markdown preview — anchors to the matching line of the source `.md`, or to the whole span of blocks a selection touches; input box instead of a gutter thread (§6) |
| Edit / Resolve / Reopen / Delete Comment (Markdown preview) | Right-click a comment-marked block in the built-in Markdown preview; `showQuickPick` disambiguates when the block has more than one applicable comment. Same store primitives as every other entry point (§6) |
...
Commands that require a UI-supplied context object (`addComment`, `addCommentFromPreview`, `editCommentFromPreview`, `resolveCommentFromPreview`, `reopenCommentFromPreview`, `deleteCommentFromPreview`, `resolveComment`, `reopenComment`, `deleteComment`, `editComment`, `saveComment`, `cancelEditComment`, `revealComment`, `resolveCommentInTree`, `reopenCommentInTree`, `deleteCommentInTree`) are hidden from the Command Palette (`when: false`) — invoked without their context argument, they'd fail; this was a real shipped bug (every one of them threw "cannot read properties of undefined" when reachable from the palette) before being excluded.
...
- **Comment from the Markdown preview.** Added `agentComments.addCommentFromPreview` — right-click in VS Code's built-in Markdown preview to comment on the corresponding line of the source `.md` (§6). Two things that looked like blockers turned out not to be, both verified against the shipped VS Code source rather than the (stale) issue tracker: a third-party `webview/context` item *does* appear in another extension's webview, and the preview *does* publish its own source URI via `<meta id="vscode-markdown-preview-data">` — so no active-editor heuristic or QuickPick disambiguation is needed. Deliberately no preview-side rendering of comments, and no custom preview webview.
- **Multi-line preview comments, snapped to whole blocks.** Selecting across blocks in the preview now anchors the whole span (`lineHint`..`endLineHint`, both of which already existed and needed no storage change). markdown-it's source map is block-level only, so sub-block precision is impossible — instead of pretending otherwise, the input box states the resulting line range up front. A bare right-click deliberately stays single-line, because the narrower `contentHash` reanchors more durably.
- **Coverage threshold raised from 95% to 100%.** `src/` was already at 100% statements/functions/lines once `addCommentFromPreview` was covered; closing three pre-existing partial branches (the archive cache's eviction path, the queued-render error log, the config-listener's non-matching-setting case) took branches there too, so the gate now matches the standard the suite was already holding itself to rather than trailing it by five points. Note the scope: `collectCoverageFrom` is `src/**/*.ts`, so `media/preview.js` — a browser script, not bundled into the extension — is outside it and remains covered only by the manual checklist in `preview-test/CHECKS.md`.
...
- **0.4.5 — comment presence + actions in the Markdown preview.** Added a second, independent mechanism (`extendMarkdownIt` markdown-it plugin, `src/preview/commentsMarkdownItPlugin.ts`) alongside the existing `addCommentFromPreview`, so a comment's presence is now visible in the preview itself (an author-colored dot, matching the gutter's own indicator) with click-to-expand-in-place viewing and right-click Edit/Resolve/Reopen/Delete, all delegating to the same store primitives every other surface already uses. Corrected two things in the original request while implementing: added a fourth `reopenCommentFromPreview` command (not named in the request) for symmetry with resolve everywhere else in the codebase; and added a `store.onDidChangeFile` → debounced `markdown.preview.refresh` subscription, since a comment mutation never touches the `.md` file's text and the preview has no native re-render trigger for that on its own (the request's claim that the mechanism "stays live with no extra refresh logic" holds for `previewScripts`, not for this). Also discovered and worked around: `CommentStore.loadFile`/`getArchivedComments` are async but a markdown-it core rule isn't, so a new synchronous `peekCachedComments` accessor plus a cold-cache-then-refresh path was needed; `anchor.lineHint`/`endLineHint` are 1-based here despite `token.map` being genuinely 0-based (verified against both the installed source, not assumed); and the click-to-expand panel is appended to `<body>` and positioned over the clicked block rather than inserted into the DOM at that point, since a marked block can be a `<p>`/`<li>`/`<td>`/heading, none of which can validly host an arbitrary block-level child.
...
- **Fixed `env.currentDocument` unreliability in the preview-markers plugin.** Real-device testing (markers never appeared, at all, on a fresh install) surfaced a gap that reading VS Code's source alone had missed: `env.currentDocument` is `undefined` whenever a render call was given a plain markdown string rather than a document object — confirmed via temporary diagnostic logging (`console.error` in the core rule, read from the Extension Host output channel) showing the key present in `env` but its value always missing, on every render, for a preview panel that had been revived after a window reload. Fixed with a fallback (`extension.ts` tracks the last-focused Markdown editor via `onDidChangeActiveTextEditor`, seeded from `vscode.window.activeTextEditor` at activation, and the plugin uses it whenever `env.currentDocument` is undefined) — see §6's "Which file" note. Underscores a standing rule here: reading an API's source establishes what a field *can* contain, not that a given real call path always populates it — worth a live-logging check before trusting a field is always set, not just after something breaks.
- **Fixed preview markers matching but never rendering — `inline` tokens excluded from block candidates.** Same real-device session as above, next layer down: once file resolution and the comment cache were both confirmed correct, the same diagnostic logging showed `matchCommentsToBlocks` genuinely finding matches and `attrJoin`/`attrSet` genuinely being called — yet still nothing appeared. Cause: an `inline` token always shares its parent block token's exact `.map` range (`paragraph_open`+its `inline` child, etc.), and the innermost-match tie-break rule (§6, "same-width tie... breaks toward the more deeply nested token") picked the `inline` one, since it's emitted after its parent. markdown-it's renderer special-cases `type === 'inline'` to render the token's `.children` directly and never reads that token's own `.attrs` — so the match and the attribute calls were both completely real, on a token whose attributes the renderer was always going to ignore. Fixed by excluding `inline`-typed tokens from the candidate block list before matching runs at all (§6), rather than special-casing them in the tie-break rule — the rule's logic is correct for actual block nesting (list/list-item, etc.); `inline` was never a block-nesting case in the first place, now a permanent regression test (`commentsMarkdownItPlugin.test.ts`, "marks the paragraph_open token, not its inline child..."). The temporary `console.error` diagnostic logging added while chasing both this and the `env.currentDocument` fix above has been removed now that both root causes are confirmed and fixed; the debugging path — log at each stage (file resolution → cache state → block/span computation → match results) rather than guess — is what actually found both, faster than re-reading source a third time would have.
- **Preview-marker UX overhaul, following direct feedback once markers were finally visible.** Three changes, all reported against real content: (1) the marker's `display: inline-block` participated in normal flow and visibly distorted layout — most severely for tables ("destroyed entirely") but also ordinary paragraphs — switched to `position: absolute`, which removes it from layout regardless of host element type; (2) the multi-block fallback marked *every* block a comment overlapped, painting a badge on every row of a table or every item of a list a single comment covered ("dots every single line," "a complete shambles") — changed to mark only the earliest-starting one, one badge per comment, full stop; (3) a separate leading dot plus a separate trailing count pill read as two markers fighting over the same line — merged into one badge that shows a plain dot for one comment or the count digit(s) for more, via `content: attr(data-agent-comment-count)` swapped in by an attribute selector. Also added row-scoped right-click inside the expand panel (a distinct `webviewSection`, `'agentCommentsPanelRowTarget'`) so acting on a specific comment no longer requires right-clicking an ambiguous line and picking from a quickpick — reported as "inhumane." Checked, not assumed, that true left-click action buttons aren't possible: `data-vscode-context` only wires to the native context-menu system, and browsers don't honor a script-dispatched `contextmenu` event for security reasons, so row-scoped right-click is the achievable version of that ask. See §6 for the current design in full.
- **Fixed acting on one comment closing the whole expand panel, hiding untouched comments in it too.** Reported immediately after the UX overhaul above shipped. Root cause: any store change (including one made through the panel itself) triggers a full `markdown.preview.refresh`, which replaces every rendered element — including whichever one the panel was anchored to — so the panel's `openHost` reference always goes stale after any action, not just ones affecting that block. Fixed by tracking the actual comment ids on display (`openCommentIds`) instead of only the host element, and polling (via `requestAnimationFrame`, only while a panel is open) for the moment `openHost.isConnected` goes false — the tell that a refresh just happened — at which point it looks for a newly-rendered element carrying any of the same ids and reopens the panel there with whatever's left, rather than just vanishing. No message-passing or MutationObserver needed; `previewScripts` gives no direct signal for "a refresh just happened," so detecting it via the host's own connectedness was the simplest reliable option available. `media/preview.js` remains outside Jest's coverage scope (§11) — verified by manual walkthrough (`preview-test/CHECKS-comment-actions.md`), not a unit test.
- **Moved the marker off the block entirely, onto its own DOM element — CSS positioning inside the block was never going to be safe for tables.** Reported again after the previous fix: `position: absolute` still pushed rendered tables around. Table row/cell positioning has cross-engine quirks in how `position` is handled on or inside them that a from-first-principles CSS-spec argument ("absolute removes it from layout, so it's harmless") didn't predict, and this was now the second CSS-only attempt to fail against the same real symptom. Rather than keep tuning CSS values against a rendering engine this session can't run, the badge became a genuinely separate `<span>` — built and positioned by `preview.js` via `getBoundingClientRect()`, appended to `<body>`, the same technique the click-to-expand panel already used for the same underlying reason (a marked block can be a `<p>`/`<li>`/`<td>`/heading/`<tr>`, none of which can safely host arbitrary generated content without side effects). A marker living entirely outside a table's DOM subtree cannot affect its layout, categorically — not a claim that needed re-verifying against a specific engine's quirks, since it doesn't depend on any. `commentsMarkdownItPlugin.ts` needed no changes: `class="agent-comment-line"` plus its `data-agent-comment-*` attributes are now a pure query-selector hook for `preview.js`, carrying no styling of their own. This also gave the badge its own `webviewSection` (`'agentCommentsMarkerTarget'`, replacing the old `'agentCommentsPreviewTarget'` reuse for actions) — right-clicking real content now offers only Add Comment, right-clicking the badge offers the four actions, with no overlap to gate around anymore since they're no longer the same element. Kept in sync across re-renders via one `MutationObserver` on `document.body` (disconnected for the duration of its own resync so appending markers/reopening the panel doesn't retrigger itself), which also now drives the panel-reconnect logic from the previous entry — replacing that entry's `requestAnimationFrame` poll, since both problems are really the same signal ("the rendered content just changed").
