...
- **Visual language reused, not reinvented — and non-intrusive by construction.** The marker is one small circular badge in the exact colors the gutter's author-avatar SVGs already use (`#3794ff` user / `#f14c4c` agent — reproduced as flat CSS shapes rather than routed through the preview's asset loading, since the source SVGs are themselves flat single-color circles); a two-tone split when a block has both a user and an agent comment. **`position: absolute`, not `display: inline-block`** — an early version used inline-block, which participates in normal flow and, discovered against a real document, visibly pushed table columns and paragraph text around ("tables get destroyed entirely," reported directly). Absolute positioning removes it from layout entirely, so it can never do that, regardless of host element type. Floats just above-left of the block (`top: -7px; left: -7px`) rather than sitting inline before the text. One badge, not two: it shows a plain dot when the block has one comment, or the count digit(s) via `content: attr(data-agent-comment-count)` when it has more — replacing an earlier version that had a separate leading dot *and* a separate trailing count pill on the same line, which read as two competing markers "trapping" the block's content. Resolved-comment visibility respects the existing `agenticComments.editor.hideResolvedComments` setting, the same as the gutter. The native `title` attribute carries a plain-text summary for hover-to-read, no custom tooltip component.
...
- **Right-click actions: Edit / Resolve / Reopen / Delete**, wired to the exact same store primitives the gutter and sidebar already call (`store.updateCommentText`, `store.resolveComment`, `store.reopenComment`, `store.deleteComment`) — no new comment-mutation logic. **Reopen** wasn't in the original request for this feature, which named only edit/resolve/delete; added anyway, since resolve/reopen are a symmetric pair at every other entry point in this codebase (gutter, sidebar), and without it a resolved comment shown in the preview (`hideResolvedComments: false`) would have no way back to unresolved except leaving the preview. **Edit** can't literally reuse the gutter's `editComment`/`saveComment` (those flip a live `CommentThread`'s comment into `CommentMode.Editing` — a construct that doesn't exist in a webview); it reuses the primitive underneath that instead (`store.updateCommentText`, which itself only ever looks at live/unresolved comments — so edit is unresolved-only here too, same as the gutter), via an input box prefilled with the current text, mirroring `addCommentFromPreview`'s own input-box pattern. All resolve which file the same way `addCommentFromPreview` already does (the preview's own embedded `source` URI, no guessing) — one shared helper, not several copies.
  - **Two right-click entry points, same commands, same controller code.** (1) Right-click the badge/block itself — carries every comment id on that block; if more than one is applicable, `showQuickPick` disambiguates (label = truncated text, description = author + status), a lone candidate acts without asking. (2) Right-click a *specific row* inside the already-open expand panel — carries just that one id, so the quickpick never appears at all. Reported directly as much better UX than picking a target out of a list after right-clicking an ambiguous line. Both wire to the identical `editCommentFromPreview`/etc. commands and `resolvePreviewCommentTarget` controller logic; a row's context payload is just a single-id case of the same shape the block-level one already produced, so no controller changes were needed — only `preview.js` (a distinct `webviewSection`, `'agentCommentsPanelRowTarget'`, set on `contextmenu` when the target is inside `.agent-comment-panel-entry`) and `package.json` (broadened the four action commands' `when` clauses to accept either section; `addCommentFromPreview`'s stays scoped to the block section only — adding a comment from our own summary panel's chrome doesn't make sense). `package.json`'s `webview/context` `when` clauses still pre-filter which actions even appear (Resolve only when there's an unresolved candidate in scope, Reopen only a resolved one) using two boolean-ish flags the plugin/preview.js compute per scope — the controller still does its own precise per-id filtering once an action actually runs, since a stale marker could theoretically lag one render behind a change made elsewhere.
