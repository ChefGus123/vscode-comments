...
- **Visual language reused, not reinvented.** The marker is a small dot in the exact colors the gutter's author-avatar SVGs already use (`#3794ff` user / `#f14c4c` agent — reproduced as flat CSS circles rather than routed through the preview's asset loading, since the source SVGs are themselves flat single-color circles), one dot per distinct author type present (so up to two), plus a small count badge past one comment. Resolved-comment visibility respects the existing `agenticComments.editor.hideResolvedComments` setting, the same as the gutter — when on, a resolved comment contributes no marker at all. The native `title` attribute carries a plain-text summary for hover-to-read, no custom tooltip component.
