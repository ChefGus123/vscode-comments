...
Right-clicking a block in VS Code's *built-in* Markdown preview offers **Add Comment**, which anchors to the corresponding line of the source `.md`. Comments are deliberately **not rendered inside the preview** — they show up in the normal editor gutter and the sidebar, same as any other comment. We extend the built-in preview only; shipping our own preview webview is a non-goal (§2).
